...
# Configurar la conexión con Google Sheets usando el enlace compartido
scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
creds = Credentials.from_service_account_file("path/to/your/credentials.json", scopes=scope)
client = gspread.authorize(creds)
sheet = client.open_by_url("https://docs.google.com/spreadsheets/d/1YKCNUTHLiS3gW7Lt2y8TGCLHh2FrkuZFOjkruaIeJro/edit?usp=sharing").sheet1
...
# Leer el último ID usado desde la hoja de cálculo
def get_last_trip_id():
    records = sheet.get_all_records()
    if records:
        return max([int(record['trip_id']) for record in records])
    return 0
...
# Guardar datos de eventos al salir en Google Sheets
for event in events:
    row = [
        trip_id,
        driver_name,
        ...
        event['event'],
        event['time'],
        event.get('total_time', ''),
        event.get('total_pause_time', ''),
        event.get('fare', ''),
        event.get('initial_distance', ''),
        event.get('final_distance', ''),
        event.get('total_distance', '')
    ]
